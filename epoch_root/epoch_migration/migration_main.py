...
def __main():
    """epoch migration main

    Returns:
        int: result(0=succeed / other=failed)
    """
    try:
        globals.logger.info('START : epoch migration_main')

        # database initialize
        migration_init_db.migration_init_db()

        # get database data version
        with closing(migration_common.connect_epoch_db()) as conn:
        ...
        if system_version == db_data_version:
            ...
            globals.logger.info(f'SKIP : epoch migration_main : Data and System versions match : {system_version}')
            return 0
        ...
        migrations = [os.path.basename(f).replace('_', '.') for f in os.listdir(migrations_path) if os.path.isdir(os.path.join(migrations_path, f)) and os.path.isfile(os.path.join(migrations_path, f, 'migration.py'))]
        ...
        globals.logger.info('SUCCEED : epoch migration_main')
        return 0
