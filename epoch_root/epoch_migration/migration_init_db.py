...
from libs import migration_init_db_queries
...
def migration_init_db():
    ...
    """
    try:
        globals.logger.info('START : migration_init_db')
        ...
        with closing(
            ...
            if db_data_version is not None:
                globals.logger.info(f'SKIP : migration_init_db : alredy initialized (db data version : {db_data_version})')
                return 0

            with conn.cursor() as cursor:

                for query_template in migration_init_db_queries.INITIALIZE_QUERIES:
        ...
        globals.logger.info('SUCCEED : platform migration_init_db')
        return 0
